...
if __name__ == '__main__':
    ...
    match value:
        ...
        case 2:
            ...
            result = ppi_from_pixel_distance(pixel_distance(distance))
            print('观看距离为{:.1f}厘米时，最佳ppi约为{:.2f}'.format(distance, result))
        case 3:
            h_px = int(input('请输入横向像素数量'))
            v_px = int(input('请输入纵向像素数量'))
            d_inch = float(input('请输入屏幕对角线的长度，单位为英寸'))
            ppi_value = ppi(h_px, v_px, d_inch)
            result = view_distance(pixel_distance_from_ppi(ppi_value))
            print('分辨率为{}x{}的{}寸屏幕，最短（近）观看距离约为{:.2f}厘米'
            .format(h_px, v_px, d_inch, result))
        case 4:
            pass
        case _:
            print('输入的序号有误')